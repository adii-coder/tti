...
# Streamlit UI Configuration
st.set_page_config(page_title="Rachna - AI Image Creator", layout="wide")
...
def toggle_mode():
    if st.session_state.mode == "Image Generation":
        st.session_state.mode = "Image Enhancement"
    else:
        st.session_state.mode = "Image Generation"

st.sidebar.button(f"🔄 {st.session_state.mode}", on_click=toggle_mode)
...
if st.session_state.mode == "Image Generation":
            ...
            try:
                ...
                for i in range(num_variations):
                    ...
                    with cols[i]:
                        ...
                        img_bytes = io.BytesIO()
                        generated_image.save(img_bytes, format="PNG")
                        st.download_button(f"💽 Download {i+1}", img_bytes.getvalue(), file_name=f"image_{i+1}.png")
    ...
    if uploaded_file:
        ...
        if st.button("✨ Enhance Image"):
            ...
            img_bytes = io.BytesIO()
            enhanced_image.save(img_bytes, format="PNG")
            st.download_button("💽 Download Enhanced Image", img_bytes.getvalue(), file_name="enhanced_image.png")
            st.markdown("---")
            st.markdown("🔹 **Powered by Stable Diffusion** | Created with ❤️ by AI Enthusiasts ADITYA TIWARI")
...
if "history" in st.session_state and st.session_state.history:
    for idx, img_bytes in enumerate(st.session_state.history[-5:]):
        st.sidebar.image(io.BytesIO(img_bytes), caption=f"History {idx+1}", use_container_width=True)
        st.sidebar.download_button("💽 Download", img_bytes, file_name=f"history_{idx+1}.png")
if st.sidebar.button("🗑️ Clear History"):
    st.session_state.history = []
