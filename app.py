...
client = InferenceClient("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")  # Replace with your API key
...
def enhance_image(image, options):
    ...
    if "Studio Ghibli" in options:
        # Sending image to API for Ghibli-style enhancement
        try:
            with st.spinner("⏳ Generating Studio Ghibli-style image... Please wait!"):
                enhanced_image = client.image_to_image(
                    img_bytes, 
                    prompt="Studio Ghibli-style animation with vibrant colors and soft lighting"
                )
            return Image.open(io.BytesIO(enhanced_image))  # Convert response to Image
        except Exception as e:
            st.error(f"❌ Ghibli Enhancement Error: {e}")
            return image  # Return original image if enhancement fails

    # Apply normal enhancements
    if "Sharpen" in options:
        image = ImageEnhance.Sharpness(image).enhance(4.0)
    if "Contrast" in options:
        image = ImageEnhance.Contrast(image).enhance(2.5)
...
# Streamlit UI
st.set_page_config(page_title="AI Image Enhancement", page_icon="🎨", layout="wide")
st.title("✨ AI Image Enhancement & Ghibli Effect")
...
if uploaded_file:
    ...
    st.write("### Select Enhancements:")
    enhance_options = st.multiselect(
        "Choose enhancements to apply:",
        ["Studio Ghibli", "Sharpen", "Contrast", "Brightness", "Saturation", "Grayscale", "HDR Effect"]
    )
    ...
    if st.button("Enhance Image"):
        if enhance_options:
            ...
            st.warning("⚠️ Please select at least one enhancement.")
